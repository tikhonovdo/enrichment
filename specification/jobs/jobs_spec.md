...
```
3. Прогнать [матчинг категорий](matchingategory_matching.md) для наполнения таблицы <u>category_matching</u> полями:
   * `bank_category_name`
   * `bank_id`
...
8. Выполнить  [матчинг переводов](matchingransfer_matching.md)
